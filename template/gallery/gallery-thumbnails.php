<div
  ...
  <?php foreach (array_merge($attrs, [
    'id' => $id . '-thumbnails',
    'class' => 'mt-2 d-grid column-gap-2 overflow-hidden',
    'style' => "grid-template-columns: repeat(auto-fill, minmax({$column_width}px, 1fr)); grid-template-rows: 1fr; grid-auto-rows: 0;"
  ]) as $name => $value): ?>
    <?= $name ?>="<?= $value ?>"
  <?php endforeach; ?>
>
<?php while( have_posts()) : the_post() ?>
  <?= wp_get_attachment_image(get_the_ID(), 'small', false, [
    ...
    'style' => "aspect-ratio: 1/1; border-radius: var(--bs-border-radius); cursor: pointer; height: 100%; max-height: 175px; width: auto",
...
<script>
  (() => {
    ...
    const resize = () => {
      ...
      const w = thumbnails.clientWidth + g;
